...
const filterData = (req) => {
  ...
  if (req.query.created_sdg) {
    $where["created_sdg"] = {
      gte: new Date(req.query.created_sdg + " 00:00:00").toISOString(),
      lte: new Date(req.query.created_sdg + " 23:59:00").toISOString(),
    };
  }
...
const methods = {
  ...
  async onGetAll(req, res) {
    try {
      ...
      const item = await prisma.sdg.findMany({
        select: selectField,
        where: $where,
  ...
  async onGetById(req, res) {
    try {
      const item = await prisma.sdg.findUnique({
        select: selectField,
        where: {
