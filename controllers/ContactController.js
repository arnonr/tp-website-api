...
const filterData = (req) => {
    ...
    if (req.query.created_contact) {
        $where["created_contact"] = {
            gte: new Date(
                req.query.created_contact + " 00:00:00"
            ).toISOString(),
            lte: new Date(
                req.query.created_contact + " 23:59:00"
            ).toISOString(),
        };
    }
...
const countDataAndOrder = async (req, $where) => {
    ...
    //Count
    let $count = await prisma.contact.findMany({
        where: $where,
    });
...
const methods = {
    ...
    async onGetAll(req, res) {
        try {
            ...
            const item = await prismaLang.contact.findMany({
                select: selectField,
                where: $where,
    ...
    async onGetById(req, res) {
        try {
            let prismaLang = checkLanguage(req);
            const item = await prismaLang.contact.findUnique({
                select: selectField,
                where: {
